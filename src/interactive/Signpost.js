...
/**
 * Signposts: `<chart-signpost>`.
 *
 * One click plants a pole at the pointer with a text box riding the top —
 * the chart-side of TradingView's signpost. Each signpost keeps its own
 * text; editing it is application UI, hand back the list with new words.
 */
export class Signpost extends ElementBase {
    #props
    #override = null
    ...
    constructor() {
        super()
        this.#props = defineProperties(this, signpostDefaults)

        this.terminate = terminate.bind(this)
        this.saveNodeType = saveNodeType.bind(this)
        this.getSelectionState = isHoverForInteractiveType("signposts").bind(this)
    }
    ...
    #build() {
        ...
        while (this.#wrappers.length > props.signposts.length) this.#wrappers.pop().remove()
        while (this.#wrappers.length < props.signposts.length) {
            const wrapper = document.createElement("chart-each-signpost")
        ...
        props.signposts.forEach((each, index) => {
            ...
            Object.assign(this.#wrappers[index], {
                ...
                onDragComplete: this.#handleDragSignpostComplete,
            })
    ...
    /** One click, one signpost — the text of the moment is frozen into it. */
    ...
    #handleDragSignpost = (event, index, newValues) => {
        this.setInteractiveState({ override: { index, ...newValues } })
    }

    #handleDragSignpostComplete = (event, moreProps) => {
        if (!isDefined(this.#override)) return

        const { index, at } = this.#override

        const newSignposts = this.#props.signposts.map((each, position) =>
...
define("chart-signpost", Signpost)
