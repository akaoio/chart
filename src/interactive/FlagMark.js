...
/**
 * Flag marks: `<chart-flag-mark>`.
 *
 * One click plants a ⚑ at the pointer — the same glyph-on-a-text-box ride
 * the arrow marks use, so the wrapper is shared and only the glyph and its
 * color differ.
 */
export class FlagMark extends ElementBase {
    #props
    #override = null
    ...
    constructor() {
        super()
        this.#props = defineProperties(this, flagMarkDefaults)

        this.terminate = terminate.bind(this)
        this.saveNodeType = saveNodeType.bind(this)
        this.getSelectionState = isHoverForInteractiveType("flags").bind(this)
    }
    ...
    #build() {
        ...
        while (this.#wrappers.length > props.flags.length) this.#wrappers.pop().remove()
        while (this.#wrappers.length < props.flags.length) {
        ...
        props.flags.forEach((each, index) => {
            ...
            Object.assign(this.#wrappers[index], {
                ...
                glyph: "⚑",
                fill: appearance.flagFill,
                appearance: {
                    bgFill: appearance.bgFill,
                    fontFamily: appearance.fontFamily,
                    fontSize: appearance.fontSize,
                },
                hoverText: { ...flagMarkDefaults.hoverText, ...props.hoverText },
                onDrag: this.#handleDragFlag,
                onDragComplete: this.#handleDragFlagComplete,
            })
    ...
    #handlePlace = (event, xyValue, moreProps) => {
        ...
        const newFlags = [
            ...this.#props.flags.map(each => ({ ...each, selected: false })),
            { at: xyValue, selected: true, appearance: this.#props.appearance },
        ]

        this.#props.onComplete?.(event, newFlags, moreProps)
    }

    #handleDragFlag = (event, index, newValues) => {
        this.setInteractiveState({ override: { index, ...newValues } })
    }

    #handleDragFlagComplete = (event, moreProps) => {
        if (!isDefined(this.#override)) return

        const { index, at } = this.#override

        const newFlags = this.#props.flags.map((each, position) =>
...
define("chart-flag-mark", FlagMark)
