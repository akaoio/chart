...
export class Pin extends ElementBase {
    ...
    constructor() {
        super()
        this.#props = defineProperties(this, pinDefaults)

        this.terminate = terminate.bind(this)
        this.saveNodeType = saveNodeType.bind(this)
        this.getSelectionState = isHoverForInteractiveType("pins").bind(this)
    }
    ...
    #build() {
        ...
        while (this.#wrappers.length > props.pins.length) this.#wrappers.pop().remove()
        while (this.#wrappers.length < props.pins.length) {
        ...
        props.pins.forEach((each, index) => {
            ...
            Object.assign(this.#wrappers[index], {
                ...
                hoverText: { ...pinDefaults.hoverText, ...props.hoverText },
                onDrag: this.#handleDragPin,
                onDragComplete: this.#handleDragPinComplete,
            })
    ...
    #handlePlace = (event, xyValue, moreProps) => {
        ...
        const newPins = [
            ...this.#props.pins.map(each => ({ ...each, selected: false })),
            { at: xyValue, selected: true, appearance: this.#props.appearance },
        ]

        this.#props.onComplete?.(event, newPins, moreProps)
    }

    #handleDragPin = (event, index, newValues) => {
        this.setInteractiveState({ override: { index, ...newValues } })
    }

    #handleDragPinComplete = (event, moreProps) => {
        if (!isDefined(this.#override)) return

        const { index, at } = this.#override

        const newPins = this.#props.pins.map((each, position) =>
