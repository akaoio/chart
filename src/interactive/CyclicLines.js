...
export const cyclicLinesDefaults = {
    enabled: true,
    snap: false,
    snapTo: undefined,
...
/**
 * Cyclic lines: `<chart-cyclic-lines>`.
 *
 * Hai cú bấm định một chu kỳ — vạch dọc lặp từ điểm đầu về phía phải cho
 * tới mép domain. Kéo tay cầm đổi chu kỳ, kéo vạch dời cả bộ.
 */
export class CyclicLines extends ElementBase {
    #props
    #current = null
    #override = null
    #mouseMoved = false
    ...
    constructor() {
        super()
        this.#props = defineProperties(this, cyclicLinesDefaults)

        this.terminate = terminate.bind(this)
        this.saveNodeType = saveNodeType.bind(this)
        this.getSelectionState = isHoverForInteractiveType("cycles").bind(this)
    }
    ...
    #build() {
        ...
        while (this.#wrappers.length > props.cycles.length) this.#wrappers.pop().remove()
        while (this.#wrappers.length < props.cycles.length) {
        ...
        props.cycles.forEach((each, index) => {
            ...
            Object.assign(this.#wrappers[index], {
                ...
                appearance,
                hoverText: { ...cyclicLinesDefaults.hoverText, ...props.hoverText },
                onDrag: this.#handleDragCycles,
                onDragComplete: this.#handleDragCyclesComplete,
            })
    ...
    #handleEnd = (event, xyValue, moreProps) => {
        ...
        const newCycles = [
            ...this.#props.cycles.map(each => ({ ...each, selected: false })),
        ...
        this.#props.onComplete?.(event, newCycles, moreProps)
    }

    #handleDragCycles = (event, index, newPoints) => {
        this.setInteractiveState({ override: { index, ...newPoints } })
    }

    #handleDragCyclesComplete = (event, moreProps) => {
        if (!isDefined(this.#override)) return

        const { index, start, end } = this.#override

        const newCycles = this.#props.cycles.map((each, position) =>
...
define("chart-cyclic-lines", CyclicLines)
