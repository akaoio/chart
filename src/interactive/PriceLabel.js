...
export const priceLabelDefaults = {
    enabled: true,
...
/**
 * Price labels: `<chart-price-label>`.
 *
 * One click plants a box whose text is the price under the pointer — and
 * stays honest afterwards: the text is derived from the label's own y on
 * every draw, so dragging it re-prices it. Nothing to edit, nothing to lie.
 */
export class PriceLabel extends ElementBase {
    #props
    #override = null
    ...
    constructor() {
        super()
        this.#props = defineProperties(this, priceLabelDefaults)

        this.terminate = terminate.bind(this)
        this.saveNodeType = saveNodeType.bind(this)
        this.getSelectionState = isHoverForInteractiveType("labels").bind(this)
    }
    ...
    #build() {
        ...
        while (this.#wrappers.length > props.labels.length) this.#wrappers.pop().remove()
        while (this.#wrappers.length < props.labels.length) {
            const wrapper = document.createElement("chart-each-price-label")
        ...
        props.labels.forEach((each, index) => {
            ...
            Object.assign(this.#wrappers[index], {
                ...
                onDragComplete: this.#handleDragLabelComplete,
            })
    ...
    #handlePlace = (event, xyValue, moreProps) => {
        ...
        const newLabels = [
            ...this.#props.labels.map(each => ({ ...each, selected: false })),
            { at: xyValue, selected: true, appearance: this.#props.appearance },
        ]

        this.#props.onComplete?.(event, newLabels, moreProps)
    }

    #handleDragLabel = (event, index, newValues) => {
        this.setInteractiveState({ override: { index, ...newValues } })
    }

    #handleDragLabelComplete = (event, moreProps) => {
        if (!isDefined(this.#override)) return

        const { index, at } = this.#override

        const newLabels = this.#props.labels.map((each, position) =>
...
define("chart-price-label", PriceLabel)
